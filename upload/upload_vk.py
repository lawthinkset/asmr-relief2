...
def upload_to_vk(video_path, description="", title=""):
    ...
    try:
        ...
        # Prepare message
        message = description if description else "✨ Deep Tingles & Relaxation\n\n#asmr #satisfying #triggers #tingles"
        
        # Ensure message is not empty (VK requirement)
        if not message.strip():
            message = "✨ New ASMR Video!"
        ...
        video = upload.video(
            ...
            name=title or 'Relaxing ASMR Video',
...
def main():
    ...
    description = sys.argv[2] if len(sys.argv) > 2 else "Deep Tingles & Relaxation ✨"
    title = sys.argv[3] if len(sys.argv) > 3 else "Relaxing ASMR"
